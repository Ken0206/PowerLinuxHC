...
find ahc -name df_h > ${tmpFile2}	#尋找並列出檔案清單存檔
echo -e "\nDisk usage:\n"
...
do
    ...
    while read LINE ; do
        ...
        if [ ${usageVal} -gt 95 ] && [ ${remainingSpaceM} == M  ] && [ ${remainingSpace} -lt 256 ] ; then	#使用率大於95% 且 可用空間單位是 M 且 可用空間小於 256
            if [ ${hostV} -eq 0 ] ; then	#是否顯示 hostname
                echo ${hostName}
                hostV=1
            fi
